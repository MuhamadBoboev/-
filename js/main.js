...
const numberQuestion = document.querySelectorAll(".number-1");
0;
...
button.addEventListener("click", function () {
  if (
    numberQuestion[0].value === numberAnswer[0].value &&
    Number(numberQuestion[0].value) > 0
  ) {
  ...
  if (counter.innerText == 0) {
    ...
    for (let a in numberQuestion) {
      overBlock.querySelector(".ovet__answer").innerText =
        numberQuestion[0].value;
    }
  } else {
    mainBlock.style.display = "inherit";
    overBlock.style.display = "none";
  }
  const overText = (overBlock.querySelector(".ovet__answer").innerText =
    numberQuestion[0].value +
    numberQuestion[1].value +
    numberQuestion[2].value +
    numberQuestion[3].value);
...
function ifen() {
  function BD(a, b) {
    if (
      numberQuestion[a].value === numberAnswer[b].value &&
      Number(numberQuestion[a].value) > 0
    ) {
      document.querySelectorAll(`.win-${a + 1}`)[0].style.backgroundColor =
        "green";
      document.querySelectorAll(`.win-${a + 1}`)[1].style.backgroundColor =
        "green";
